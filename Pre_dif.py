...
def find_largest_n(matrix, n):
    ...
    # اعمال np.argmax() بر روی هر سطر جداگانه
    max_columns = np.argsort(matrix)[-n:]
    num_neg_200 = np.sum(matrix == -200)
    return largest_n, max_columns, num_neg_200

n = 8
X_train_1 = np.zeros((X_train.shape[0], n*2 + 1))  # تعیین ابعاد مناسب برای X_train_1
for i in range(X_train.shape[0]):
    temp = X_train[i, :]
    largest_n, max_columns, num_neg_200 = find_largest_n(temp, n)
    X_train_1[i, :n] = largest_n[0]  # اختصاص دادن عنصر اول از بزرگترین
    X_train_1[i, n:n*2] = max_columns  # اختصاص دادن شماره‌های ستونی
    X_train_1[i, n*2] = num_neg_200  # اختصاص دادن تعداد عناصر -200
X_train = X_train_1

X_train_1 = np.zeros((X_test.shape[0], n*2 + 1))  # تعیین ابعاد مناسب برای X_train_1
