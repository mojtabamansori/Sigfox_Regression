...
def f_e_mean_std(input_model, output_model, n_s,sl):
    ...
    for section in range(n_s):
        ...
        if section == 0:
            data_getway = input_model[(((sl[0,0]) < index_Y) & ((sl[0,1]) > index_Y))]
        ...
        if section == 1:
            data_getway = input_model[(((sl[1, 0]) < index_Y) & ((sl[1, 1]) > index_Y))]
            mean_getway = np.mean(data_getway, axis=0)
            std_getway = np.std(np.float32(data_getway), axis=0)
            data_plot_mean[section, :] = mean_getway
            data_plot_std[section, :] = std_getway
